...
class _FloatingMapPreviewState extends State<FloatingMapPreview> with SingleTickerProviderStateMixin {
  ...
  void _initPosition() {
    final restoredYPosition = _userSettings.previewMapY * _containerSize.height;

    if (_userSettings.bIsPreviewMapDocked) {
      switch (_userSettings.previewMapSide) {
  ...
  void _movePreviewTarget(Offset newTarget, {required SpringDescription springDescription}) {
    // Clamp vertical axis in a valid range
    final double clampedY = _getClampedPreviewYPosition(newTarget.dy);

    _targetPreviewPosition = Offset(newTarget.dx, clampedY);
  ...
  void _dockPreview(_DockSide side, {bool moveOffscreen = true, double? dockY}) {
    ...
    if (_bIsUndockedOrUndocking) {
      ...
      switch (side) {
        case _DockSide.left:
          _userSettings.previewMapSide = PreviewMapSide.left;
          break;

        case _DockSide.right:
          _userSettings.previewMapSide = PreviewMapSide.right;
          break;
      ...
      _userSettings.bIsPreviewMapDocked = true;
  ...
  void _undockPreview() {
    if (_bIsDockedOrDocking) {
      ...
      _userSettings.bIsPreviewMapDocked = false;
    }
  }
  ...
  void _updateSettingsPreviewMapY(double dockY) {
    _userSettings.previewMapY = dockY / _containerSize.height;
  }
...
class _DockTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: IconTheme(
        data: Theme.of(context).iconTheme.copyWith(color: Theme.of(context).colorScheme.onSurface),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.map),
          SizedBox(
            height: 6,
          ),
          Icon(bIsFacingLeft ? Icons.chevron_left_rounded : Icons.chevron_right_rounded),
        ]),
      ),
    );
  }
