...
/// The types of actor that can be displayed as a pin on the map.
enum _PinType {
  Lightcard,
  ColorCorrectWindow,
}
...
class _StageMapState extends State<StageMap> with PreviewRenderConsumer {
  ...
  /// The user settings to use.
  late LightcardTabUserSettings _userSettings;

  /// The path of the last root actor that this was displaying.
  String? _lastRootActorPath;
  ...
  Widget build(BuildContext context) {
    if (!widget.bForceShowFullMap) {
      _mapTransform.value = _userSettings.stageMapTransform;
    }
      ...
      child: LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
        ...
        if (_previewImage == null) {
          ...
          if (_connectionManager.connectionState == EngineConnectionState.connected &&
              _userSettings.displayClusterRootPath != null) {
        ...
        // Create the pins. Draggable (used in the pin) doesn't play nicely when its containing stack is reordered, so
        // don't try to reorder or remove items from this list in the middle of a drag operation.
        final Map<_StageMapPin, Offset> pinLocations = {};
        for (MapEntry<String, _MapActorData> actorPair in _mapActorDataByPath.entries) {
          if (widget.bShowOnlySelectedPins && !_userSettings.isActorSelected(actorPair.key)) {
            continue;
          }
          ...
          late final _PinType pinType;
          if (actor.isA(lightCardClassName)) {
            pinType = _PinType.Lightcard;
          } else if (actor.isAny(colorCorrectWindowClassNames)) {
            pinType = _PinType.ColorCorrectWindow;
          } else {
            throw 'No pin type for actor ${actor.path}';
          }
          ...
          final _StageMapPin pin = widget.bIsControllable
              ? _StageMapPin(
                  actorPath: actorPair.key,
                  pinType: pinType,
                  bIsDraggable: !_userSettings.bIsInTransformMode,
            ...
            children: [
                  ...
                  child: Padding(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: UnrealColors.gray1.withOpacity(0.8),
                      ),
                      width: 200,
                      child: Column(
                        ...
                        children: [
                          Text(
                            'TEMPORARY UI',
                            style: Theme.of(context).textTheme.bodyText2!.copyWith(color: Colors.amber),
                          ),
                          // Projection mode control
                          DropdownButton<ProjectionMode>(
                            value: _userSettings.projectionMode,
                            isExpanded: true,
                            hint: const Text('Projection Mode'),
                            items: [
                              for (ProjectionMode mode in ProjectionMode.values)
                                DropdownMenuItem<ProjectionMode>(
                                  value: mode,
                                  child: Text(_getProjectionModeName(mode)),
                                )
                            ],
                            onChanged: (ProjectionMode? value) {
                              if (value != null) {
                                _userSettings.projectionMode = value;
                              }
                            },
                          ),
                          ...
                          FloatingActionButton.extended(
                            icon: Icon(_userSettings.bIsInTransformMode ? Icons.pin_drop : Icons.map),
                            label: _userSettings.bIsInTransformMode ? const Text('Objects') : const Text('Map'),
                            onPressed: _toggleTransformMode,
                            backgroundColor: CupertinoDynamicColor.resolve(
                              _userSettings.bIsInTransformMode
              ...
              if (_bShowRealTimeDisabledWarning)
                const Padding(
                  ...
                  child: Text(
                    'WARNING: Real-time rendering is disabled in the editor.\n'
                    'nDisplay previews and light card positions may not update correctly.',
                    style: TextStyle(
                      color: Colors.yellow,
  ...
  void initState() {
    ...
    _mapTransform.addListener(_saveMapTransform);
  ...
  void dispose() {
    ...
    _mapTransform.removeListener(_saveMapTransform);
  ...
  void onPreviewRenderCompleted({
    ...
    if (_sequenceNumber >= _sequenceNumber) {
  ...
  void _updateConstraints(BoxConstraints constraints) {
    ...
    if (newSize != _constraintSize) {
      ...
      if (_constraintSize.height > 0 && _constraintSize.width > 0) {
        final double aspectRatio = _constraintSize.width / _constraintSize.height;

        final double newWidth = math.sqrt(_previewResolutionPixels * aspectRatio);
        _desiredPreviewSize = Size(newWidth, newWidth / aspectRatio);
      } else {
  ...
  void _onUserSettingsChanged() {
    if (_userSettings.bIsInTransformMode) {
      // Register for properties necessary to transform a light card
      _updateActorTransformRegistration();
    }
    ...
    if (_lastRootActorPath != _userSettings.displayClusterRootPath) {
      _lastRootActorPath = _userSettings.displayClusterRootPath;
      _resetRootActorSpecificData();
    }
  }
  ...
  /// If a [primaryPinPath] is provided, that pin will be the one around which other pins' locations will rotate to match the wall's angle.
  /// If no path is provided, the nearest pin to the pointer will be used as the primary, so the [pointerPosition] must be provided instead.
  void _startPinDrag(String? primaryPinPath, [Offset? pointerPosition]) {
    ...
    final List<String> draggedActors = _userSettings.selectedActors.toList();
    if (primaryPinPath == null) {
      ...
      for (final String pinPath in _userSettings.selectedActors) {
      ...
      if (!draggedActors.contains(primaryPinPath)) {
        // Make sure that the pressed pin is dragged even if it's not multi-selected
  ...
  void _onScaleStart(ScaleStartDetails details) {
    ...
    } else if (details.pointerCount > 1 || _userSettings.projectionMode == ProjectionMode.uv) {
      // Start a camera track and zoom if we're using a scale gesture or just dragging in UV mode (since it can't tumble)
  ...
  void _onLongPressStart(LongPressStartDetails details) {
    DropDownListMenu.showAtPosition(
      ...
      builder: (context) => PlaceActorDropDownMenu(actorMapPosition: _globalToMapPosition(details.globalPosition)),
    );
  }
  ...
  void _onCameraTumbleUpdate(ScaleUpdateDetails details) {
    ...
    vec.Vector2 cameraAngle = _userSettings.cameraAngle;
    ...
    _userSettings.cameraAngle = cameraAngle;
  }
  ...
  void _onTransformScaleStart() {
    ...
    if (_propertyManager.beginTransaction('Transform Stage Actors')) {
  ...
  /// Get the rectangle containing the map.
  Rect _getContainerRect() {
    return Offset.zero & _constraintSize;
  }

  /// Calculate the viewport area, i.e. the area that of that map currently filling the container, when the map is
  /// using the given transformation [matrix].
  Rect _calculateViewport(Matrix4 matrix) {
    final Rect containerRect = _getContainerRect();
  ...
  void _translateMap(Offset translation) {
  ...
  void _saveMapTransform() {
    _userSettings.stageMapTransform = _mapTransform.value;
  }
  ...
  /// Toggle transform mode on and off.
  void _toggleTransformMode() {
    _userSettings.bIsInTransformMode = !_userSettings.bIsInTransformMode;
    _updateActorTransformRegistration();
  }
  ...
  void _updateActorTransformRegistration() {
    ...
    for (final String actorPath in registeredActorPaths) {
      if (_userSettings.selectedActors.contains(actorPath)) {
        continue;
      }
    ...
    // Register new actors
    for (String actorPath in _userSettings.selectedActors) {
    ...
    if (bAnyChanges) {
      ...
      Future.wait(waitFutures).then((_) {
        ...
        _userSettings.bIsInTransformMode = false;
  ...
  Offset _getPinOffset(Offset position) {
    ...
    final double maxMapScale = _constraintSize.height / _previewSize.height;

    // How much the map will be offset left/right to be centered in the container
    final double mapXOffset = (_constraintSize.width - _previewSize.width * maxMapScale) / 2;

    final transformed = vec.Vector3(
      position.dx * maxMapScale * _previewSize.width + mapXOffset,
      position.dy * maxMapScale * _previewSize.height,
      0,
    );
  ...
  bool _shouldForcePinDragAnimation(String actorPath) {
    if (!_userSettings.bIsInTransformMode && actorPath == _draggedPinPath) {
      // When dragging a actor directly, let it control its own drag animation
      return false;
    }

    return _gestureState == _GestureState.transformDragging && _userSettings.selectedActors.contains(actorPath);
  }
...
class _StageMapPin extends StatefulWidget {
  ...
  /// The path of the actor this represents.
  final String actorPath;

  /// The type of actor represented by this pin.
  final _PinType pinType;
...
class _StageMapPinState extends State<_StageMapPin> with TickerProviderStateMixin {
  ...
  static const double _defaultPinSize = 64.0;

  /// Y offset of the pin graphic's bottom from the actual pinned location.
  static const double _bottomOffset = 4.0;
  ...
  Widget build(BuildContext context) {
    ...
    final bool bIsSelected = userSettings.isActorSelected(widget.actorPath);
  ...
  void _onTapped() {
    ...
    userSettings.selectActor(widget.actorPath, bShouldSelect: !userSettings.isActorSelected(widget.actorPath));
  }
...
class _StageMapPinIcon extends StatelessWidget {
  ...
  /// Whether the user has selected this pin.
  ...
  @override
  Widget build(BuildContext context) {
    late final String pinTypeString;
    switch (pinType) {
      case _PinType.Lightcard:
        pinTypeString = 'lightcard';
        break;

      case _PinType.ColorCorrectWindow:
        pinTypeString = 'ccw';
        break;

      default:
        throw 'No icon for pin type $pinType';
    }

    final String iconPath = 'assets/images/map/pin_$pinTypeString${bIsSelected ? '_selected' : ''}.svg';
