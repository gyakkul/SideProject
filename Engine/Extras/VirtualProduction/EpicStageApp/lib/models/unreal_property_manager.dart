...
class UnrealPropertyManager {
  ...
  void stopTrackingProperty(TrackedPropertyId propertyId, PropertyChangeCallback changeCallback) async {
    ...
    property.changeCallbacks.remove(changeCallback);
  ...
  bool beginTransaction(String description) {
    ...
    _currentTickTransactions.add(_EngineTransaction(
    ...
    for (final _EngineTransaction transaction in _currentTickTransactions) {
      if (transaction.changes.isNotEmpty) {
        if (!transaction.bWasCreatedInEngine) {
          ...
          messages.add(createUnrealWebSocketMessage('transaction.begin', {
      ...
      if (!transaction.bIsActive && transaction.bWasCreatedInEngine) {
        ...
        messages.add(createUnrealWebSocketMessage('transaction.end', {
          'TransactionId': transaction.id,
        }));
      }
    }
  ...
  void _onEngineTransactionEnded(dynamic message) {
    ...
    final int? editorSequenceNumber = message['SequenceNumber'];
    if (editorSequenceNumber == null) {
      _log.warning('Received TransactionEnded event with no sequence number');
      return;
    }

    final _EngineTransaction? activeTransaction = _getActiveTransaction();
    if (activeTransaction != null && activeTransaction.id == id) {
      // We were in the middle of a transaction and got interrupted. Mark it to be re-created with the next batch of changes.
      activeTransaction.bWasCreatedInEngine = false;
    }
  }
...
class _EngineTransaction {
  _EngineTransaction({required this.description, required this.id});

  /// The description of the transaction displayed in the editor.
  final String description;

  /// The ID used to refer to the transaction in messages to/from engine.
  final int id;
