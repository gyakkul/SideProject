...
CUresult P010_to_ABGR10(CUdeviceptr SrcI420, CUarray DstABGR8, uint32 Width, uint32 Height, uint32 Pitch)
{
	// HACK do this once instead
	const char* module_file = "Kernels/lib/P010_to_ABGR10.fatbin";
	const char* kernel_name = "p010_to_abgr10";

	CUmodule module;
	CUfunction p010_to_abgr10_func;
	CUresult Result;
	...
	Result = FCUDAModule::CUDA().cuModuleGetFunction(&p010_to_abgr10_func, module, kernel_name);
	if (Result != CUDA_SUCCESS)
	{
		UE_LOG(LogTemp, Fatal, TEXT("Get module function %s failed (%d)"), UTF8_TO_TCHAR(kernel_name), Result);
	}
	...
	Result = FCUDAModule::CUDA().cuLaunchKernel(p010_to_abgr10_func,
											   	Width / 64 + 1, Height / 64 + 1, 1,
												32, 32, 1,
												0, 0, params, NULL);
	if (Result != CUDA_SUCCESS)
